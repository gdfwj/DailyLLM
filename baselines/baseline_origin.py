        ...
        for idx, csv_file in enumerate(datasets):
            ...
            if dataset_name=="uci":
                def load_data(file_path):
                    """Load data from a text file and return as a PyTorch tensor."""
                    data = np.loadtxt(file_path)
                    return torch.tensor(data, dtype=torch.float32)
                data_dir_ = "data/extracted/uci/"
                X_train_pad = load_data(data_dir_ + "X_train.txt").reshape(-1, 561//3, 3)
                y_train_int = load_data(data_dir_ + "y_train.txt").long() - 1
                X_test_pad = load_data(data_dir_ + "X_test.txt").reshape(-1, 561//3, 3)
                y_test_int = load_data(data_dir_ + "y_test.txt").long() - 1
                max_len = len(X_train_pad[0])
                num_classes = len(set(y_train_int.tolist()))
                # print(max(y_test_int), min(y_test_int), num_classes)
            else:
                data_dir = "data/extracted/"
                if dataset_name=="HHAR":
                    train_size = 500
                ...
                # 假设原始 xyz 在 data/raw/{dataset_name}_xyz.csv
                raw_csv_path = f"{data_dir}/{dataset_name.lower()}_xyz.csv"

                X_train_pad, y_train_int, X_test_pad, y_test_int, max_len, num_classes = load_raw_xyz_data(raw_csv_path, train_size, val_size, random_state=42)
            
            print(f"原始xyz 读取完成: {dataset_name}, 训练集: {X_train_pad.shape}, 测试集: {X_test_pad.shape}")

            # 对于原始 xyz: shape=(N, max_len,6)，我们希望最终送进模型的是 (N,6,max_len) => in_channels=6
            X_train_pad = X_train.reshape(X_train.shape[0], -1, 6)
            X_test_pad  = X_test.reshape(X_test.shape[0], -1, 6)
            max_len = len(X_train_pad[0])
            num_classes = len(set(y_train_int.tolist()))
            in_channels = 6
            seq_len     = max_len
            ...
            y_pred_int, y_true_int = train_and_predict_nn(
                ...
                need_transpose=True  # 因为目前 X_train_pad 是 (N, max_len,6), 我们要先 permute(0,2,1)
            )
